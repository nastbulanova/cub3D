...
void	init_ray_steps(t_scene *scene, t_ray *ray, int *map_x, int *map_y)
{
	...
	*map_y = (int)scene->player->pos_y;
	...
	{
		ray->step_x = -1;
		ray->side_dist_x = (scene->player->pos_x - (double)*map_x) * ray->delta_dist_x;
	}
	else
	{
		ray->step_x = 1;
		ray->side_dist_x = ((double)*map_x + 1.0 - scene->player->pos_x) * ray->delta_dist_x;
	}
	if (ray->ray_dir_y < 0.0)
	{
		ray->step_y = -1;
		ray->side_dist_y = (scene->player->pos_y - (double)*map_y) * ray->delta_dist_y;
	}
	else
	{
		ray->step_y = 1;
		ray->side_dist_y = ((double)*map_y + 1.0 - scene->player->pos_y) * ray->delta_dist_y;
	}
}
...
double	calculate_wall_distance(t_scene *scene, t_ray *ray, int map_x, int map_y)
{
	...
	if (ray->side == 0)
		perp_dist = (map_x - scene->player->pos_x + (1 - ray->step_x) / 2.0) / ray->ray_dir_x;
	else
		perp_dist = (map_y - scene->player->pos_y + (1 - ray->step_y) / 2.0) / ray->ray_dir_y;
	return (perp_dist);
}
